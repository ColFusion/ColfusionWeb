...
if (strstr($_REQUEST['search'],'/') && $URLMethod == 2)
{
    ...
    $get = array();
    //$test=$_REQUEST['search'];
    foreach ($_GET as $k => $v){
    	//$get[$k] = preg_replace("/($test)/i","<b style=\"color:red\">\\1</b>",stripslashes(htmlentities(strip_tags($v),ENT_QUOTES,'UTF-8')));
...
else
{
	...
	if($rows == false){
		$main_smarty->assign('pos
			ttitle', $main_smarty->get_config_vars('PLIGG_Visual_Search_NoResults') . ' ' . stripslashes($search->searchTerm) . stripslashes($search->url));
		$main_smarty->assign('pagename', 'noresults');
	}
	...
	$pages = preg_replace("/($rows)/i", "<b style=\"color:red\">\\1</b>", $pages);
