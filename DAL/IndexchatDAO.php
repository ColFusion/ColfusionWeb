...
class IndexchatDAO {
    ...
    public function sendMessage($to_id, $message){
        ...
        $check_status = mysql_query("SELECT * FROM `index_users` WHERE id='".$to_id."' AND chat_status='online'");
        if(mysql_num_rows($check_status) > 0){

            //insert message into chat table
            $query = "INSERT INTO `index_chat` (from_id,to_id,message,sent) VALUES('".$this->user->user_id."','".$to_id."','".$message."','".time()."')";
    ...
    public function loadMessage($partner_id, $is_typing){
        ...
        $check_offline = mysql_query("SELECT chat_status FROM `index_users` WHERE id='".$partner_id."' AND chat_status='offline' AND UNIX_TIMESTAMP(NOW()) > offlineshift;");
        ...
        if(isset($is_typing)){ 
            $query = mysql_query("SELECT * FROM `index_typing` WHERE typing_from = '".$this->user->user_id."' AND typing_to = '".$partner_id."'");
            $result = mysql_fetch_assoc($query);
            $num = mysql_num_rows($query);
            if($num) {
            //update
                mysql_query("UPDATE `index_typing` SET typing_ornot = '".$is_typing."' WHERE typing_from='".$this->user->user_id."' AND typing_to = '".$partner_id."'");
            } else {
            //insert
                mysql_query("INSERT INTO `index_typing` (`typing_from`, `typing_to`, `typing_ornot`) VALUES ('".$this->user->user_id."', '".$partner_id."', '".$is_typing."')");
            }
        }

        //check if current user has unreceived messages which are older than limit, if yes, display it with date
        $check_unreceived = mysql_query("SELECT * FROM `index_chat` WHERE from_id='".$partner_id."' AND to_id='".$this->user->user_id."' AND sent < '".$sent_time."' AND recd='0' ORDER BY id");
        if(mysql_num_rows($check_unreceived) > 0){
            ...
            //there is/are an unreceived message(s)
                ...
                mysql_query("UPDATE `index_chat` SET recd='1',sent='".time()."' WHERE id='".$check_ur_row['id']."' AND recd='0'");
            }
        
            //insert info message as system into current chat
            mysql_query("INSERT INTO `index_chat` (from_id,to_id,message,sent,system_message) VALUES('".$partner_id."','".$this->user->user_id."','These are unreceived messages from the previous chat session!','".time()."','yes')");
        }

        //delete messages 2 hours before 
        $query = "DELETE FROM `index_chat` WHERE sent < ".$sent_time;
        mysql_query($query);
        //load messages
        $query = "SELECT * FROM `index_chat` WHERE (from_id='".$this->user->user_id."' AND to_id='".$partner_id."' AND sent > '".$sent_time."') OR (from_id='".$partner_id."' AND to_id='".$this->user->user_id."' AND sent > '".$sent_time."') ORDER BY sent";
    ...
    public function isTyping($partner_id){
        $res = mysql_query("SELECT typing_ornot FROM `index_typing` WHERE typing_from='".$partner_id."' AND typing_to='".$this->user->user_id."' AND typing_ornot='1'");
        if(mysql_num_rows($res) > 0){
            return '1';
        }else{
            return '0';
    ...
    public function loadPopupMessage(){
        ...
        $res = mysql_query("SELECT * FROM `index_chat` WHERE to_id='".$this->user->user_id."' AND recd='0' GROUP BY from_id LIMIT 0,1");
        if(mysql_num_rows($res) > 0){
