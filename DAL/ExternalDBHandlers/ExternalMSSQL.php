...
class ExternalMSSQL {
	...
	public static function GetConnectionFromCreds($externalDBCredentials) {
		...
		return ExternalMSSQL::GetConnection($externalDBCredentials->server_address, $externalDBCredentials->user_name,
...
//		$query = <<<EOQ
 //   select [Spd] as '$table_name.Spd', [Drd] as '$table_name.Drd', [Location] as '$table_name.Location', [AggrType] as '$table_name.AggrType', [Start] as '$table_name.Start', [End] as '$table_name.End', $colsRenamed
//		from
//		(
//			select *
//			from
...
//					for T.Dname in ($cols)
//				) as P
//		) as rot INNER JOIN (select distinct [Spd], [Drd], [Location], [AggrType], [Start], [End], rownum from [COLFUSIONDEV]...colfusion_temporary where sid = $sid) as rest ON rot.rownum = rest.rownum
//EOQ;
