...
if(!defined('mnminclude')){header('Location: ../404error.php');die();}
	...
	global $main_smarty, $cached_totals, $new_search, $key_word, $cached_links, $current_user, $cached_saved_links, $fetch_link_summary;
	$link = new Link;
...
/////
	...
	if ($new_search) {
		foreach($new_search as $link_id) {
			$link->id=$link_id;
			$link->read(True,$key_word);
			if( $display_grouplinks ) $link->link_group_id = 0;
			$link_summary_output .= $link->print_summary('summary', true);
			$link_summary_output=preg_replace("/($new_search)/i","<b style=\"color:red\">\\1</b>",$link_summary_output);  
		}
	} else {
		if ($links) {
