...
class Link {
	...
	function store() {
		...
		$htmlPath="register-wrapper/"."$Sid.ktr";
		...
		if($rs){ 
			...
			$sql = "INSERT INTO " .table_prefix."links (link_id,link_author,link_date, link_summary, link_status, link_category, link_content, link_title, link_tags, link_title_url, link_url) VALUES ('$Sid','$link_author','$EntryDate','$link_summary','queued','$link_category','$link_content','$link_title','$link_tags','$link_title_url', '$htmlPath');";
			$r=$db->query($sql);
		}

	}

	function read($usecache = TRUE, $key_word="history") {
		...
		if($link) {
			...
			//$this->title=preg_replace("/($key_word)/i","<b>$1</b>",$link->link_title);
...
/////
			...
			//$this->link_summary = preg_replace("/($key_word)/i","<b>$1</b>",$link->link_summary);
	...
	function fill_smarty($smarty, $type='full'){
		...
		if($type == "summary"){
			if($this->link_summary == ""){
				...
				$smarty->assign('story_content', "shit");
			} else {
	...
	*/	
		...
		/*	echo "select * from warehouse where Sid = '$newSid';";
			if($warehouse[])
			{
				$warehouse['Spd'];
				echo $warehouse['Spd'];
				$this->Drd=$warehouse->Drd;
				$this->From=$warehouse->From;
				$this->To=$warehouse->To;
				$this->EID=$warehouse->EID;
				$this->Value=$warehouse->Value;
				$this->AggrType=$warehouse->AggrType;
				$this->Sid=$warehouse->Sid;
				$this->LID=$warehouse->LID;
				$this->Dname=$warehouse->Dname;
				$smarty->assign('Spd', $this->Spd);
				$smarty->assign('Drd', $this->Drd);
				$smarty->assign('From', $this->From);
				$smarty->assign('To', $this->To);
				$smarty->assign('EID', $this->EID);
				$smarty->assign('Value', $this->Value);
				$smarty->assign('AggreType', $this->AggrType);
				$smarty->assign('Sid', $this->Sid);
				$smarty->assign('LID', $this->LID);
				$smarty->assign('Dname', $this->Dname);
			}
			*/
