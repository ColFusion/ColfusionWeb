...
//print_r($mail);					
	...
	function all_stats($from = false) {
		...
		$this->published_votes = $db->get_var("SELECT count(*) FROM " . table_votes . "," . table_links . " WHERE vote_user_id = $this->id AND link_id = vote_link_id AND link_status = 'published' AND vote_date < link_published_date $vote_date");
				$this->total_links = $db->get_var("SELECT count(*) FROM " . table_links . " WHERE link_author = $this->id and (link_status='private' OR link_status='queued') $link_date");
		$this->published_links = $db->get_var("SELECT count(*) FROM " . table_links . " WHERE link_author = $this->id AND link_status = 'queued' $link_date");
		$this->upcoming_links = $db->get_var("SELECT count(*) FROM " . table_links . " WHERE link_author = $this->id AND link_status = 'private' $link_date");
...
*/
		...
		if($stats == 1){		
			$this->all_stats();
			$main_smarty->assign('user_upcoming_links', $this->upcoming_links);
