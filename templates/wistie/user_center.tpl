...
<!--div class="headline">
    <div class="sectiontitle">
        <a href="{$user_url_personal_data}">{#PLIGG_Visual_Profile#}</a>
    </div>
</div-->
...
{if $user_view eq 'viewfriends'}
	...
	{if $friends}
		<table>
			...
			{if check_for_enabled_module('simple_messaging',0.6) && $is_friend}
				<th style="width:90px;text-align:left;">{#PLIGG_Visual_User_Profile_Message#}</th>
			{/if}
			...
			{foreach from=$friends item=myfriend}
				...
				<tr>
					...
					{if check_for_enabled_module('simple_messaging',0.6) && $is_friend}
						<td align="center"><a href="{$my_pligg_base}/module.php?module=simple_messaging&view=compose&return={$templatelite.server.REQUEST_URI|urlencode}&to={$myfriend.user_login}"><img src="{$my_pligg_base}/modules/simple_messaging/images/reply.png" border="0" /></a></td>
					{/if}
	...
	{else}
		...
		<h2 style="text-align:center;">
			<span style="text-transform:capitalize;">{$user_username}</span>
			{#PLIGG_Visual_User_Profile_No_Friends#}
		</h2>
	{/if}
{/if}
...
{if $user_view eq 'viewfriends2'}
		...
		<table>
			...
			{foreach from=$friends item=myfriend}
				...
				<tr>
					<td><img src="{$friend_avatar}" align="absmiddle" /> <a href="{$profileURL}">{$myfriend.user_login}</a></td>
					{if check_for_enabled_module('simple_messaging',0.6) && $is_friend}
						<td><a href="{$my_pligg_base}/module.php?module=simple_messaging&view=compose&to={$myfriend.user_login}&return={$templatelite.server.REQUEST_URI|urlencode}"><img src="{$my_pligg_base}/modules/simple_messaging/images/reply.png" border="0" /></a></td>
					{/if}
				</tr>
				{/foreach}
		</table>
	{else}
		...
		<h2 style="text-align:center;">
			<span style="text-transform:capitalize;">{$user_username}</span>
			{#PLIGG_Visual_User_Profile_No_Friends_2#}
		</h2>
	{/if}
{/if}
		...
		{if $Allow_Friends neq "0"}	
			...
			{if $user_login neq $user_logged_in}
				{if check_for_enabled_module('simple_messaging',0.6) && $is_friend}{if $friends}
					<img src="{$my_pligg_base}/modules/simple_messaging/images/reply.png" border="0" align="absmiddle" />
					<a href="{$my_pligg_base}/module.php?module=simple_messaging&view=compose&return={$templatelite.server.REQUEST_URI|urlencode}&to={$user_login}">{#PLIGG_Visual_User_Profile_Message#}
						<span style="text-transform:capitalize;">{$user_login}</span>
					</a> &nbsp;|&nbsp;
				{/if}
			{/if}
			...
			{if $is_friend gt 0}
				...
				{else}
					{if $user_authenticated eq true} 					
						<img src="{$my_pligg_base}/templates/{$the_template}/images/user_add.gif" align="absmiddle" />
						<a href="{$user_url_add}">	{#PLIGG_Visual_User_Profile_Add_Friend#}
							<span style="text-transform:capitalize;">{$user_login}</span> {#PLIGG_Visual_User_Profile_Add_Friend_2#}
						</a>
					{/if}
				{/if}
			{else}
	...
	<div id="wrapper">
		{checkActionsTpl location="tpl_pligg_profile_info_start"}
					...
					<table style="border:none">
						<tr>
							...
							<td style="background:none">
								{if $UseAvatars neq "0"}
									<span id="ls_avatar"><img src="{$Avatar_ImgSrc}" alt="Avatar" align="absmiddle" /></span>
								{/if}
								<span style="text-transform:capitalize;">{$user_username}</span>
							</td>
						</tr>			
						...
						{if $user_url ne ""}
							<tr>
								...
								<td style="white-space:nowrap; text-align:center;overflow-x: auto;width:50px;">
			...
			<div id="stats">
				<fieldset><legend>{#PLIGG_Visual_User_Profile_User_Stats#}</legend>
					<table style="border:none;">
						...
						<tr>
							<td><strong>Upcoming Stories:</strong></td>
							<td>{$user_upcoming_links}</td>
						</tr>
			...
			{if $Allow_Friends neq "0"}
				<div id="friends">
					<fieldset>
						<legend>{#PLIGG_Visual_User_Profile_Friends#}</legend>
						{if $friends}			
							<table id="friendlist">
								<thead>
									<tr>
										...
										{if $user_login eq $user_logged_in}
											<th scope="col">{#PLIGG_Visual_User_Profile_Remove_Friend#}</th>
										{/if}
									</tr>
								</thead>
								<tbody>
