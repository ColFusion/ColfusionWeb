...
<div class="register-left" style="margin-bottom: 20px;">
<form action="{$URL_register}" method="post" id="thisform">

	<h2>{#PLIGG_Visual_Register_Username#}:</h2>
	{if isset($form_username_error)}{ foreach value=error from=$form_username_error }<div class="error">{$error}</div>{ /foreach }{/if}
	...
	{if isset($form_email_error)}{ foreach value=error from=$form_email_error }<div class="error">{$error}</div>{ /foreach }{/if}
	...
	{if isset($form_password_error)}{ foreach value=error from=$form_password_error }<div class="error">{$error}</div>{ /foreach }{/if}
	...
	{/if}
        ...
        {if isset($form_validationCode_error)}
           <div class="error">{$form_validationCode_error}</div>
        {/if}
        <input id="validationCode" name="validationCode" size="25" tabindex="16" />
...
<div class="register-right" style="display: none;">
